...
class _FrameOnePageState extends State<FrameOnePage> {
  ...
  Future<void> deleteFrame(String id) async {
    try {
      final response = await http
          .delete(Uri.parse('http://192.168.1.118:8080/frame1/delete/$id'));
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  ...
                  CheckboxListTile(
                    title: Text('CO2'),
                    value: selectedMetrics.contains('CO2'),
                    onChanged: (bool? value) {
                      setState(() {
                        if (value == true) {
                          selectedMetrics.add('CO2');
                        } else {
                          selectedMetrics.remove('CO2');
                        }
                      });
                    },
                  ),
                  CheckboxListTile(
                    title: Text('TVOC'),
                    value: selectedMetrics.contains('TVOC'),
                    onChanged: (bool? value) {
                      setState(() {
                        if (value == true) {
                          selectedMetrics.add('TVOC');
                        } else {
                          selectedMetrics.remove('TVOC');
                        }
                      });
                    },
                  ),
                  CheckboxListTile(
                    title: Text('HCHO'),
                    value: selectedMetrics.contains('HCHO'),
                    onChanged: (bool? value) {
                      setState(() {
                        if (value == true) {
                          selectedMetrics.add('HCHO');
                        } else {
                          selectedMetrics.remove('HCHO');
                        }
                      });
                    },
                  ),
  ...
  Future<void> exportCSV(
    ...
    try {
      ...
      final oldFilePath = '${directory.path}/frame_data.csv';
      final tempFilePath = '${directory.path}/frame_data_temp.csv';
